    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bids.update", [$bid->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job') ? 'is-invalid' : '' }}" name="job_id" id="job_id" required>
                    @foreach($jobs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_id') ? old('job_id') : $bid->job->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('job'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bid_amount') ? 'is-invalid' : '' }}" type="number" name="bid_amount" id="bid_amount" value="{{ old('bid_amount', $bid->bid_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('points_required') ? 'is-invalid' : '' }}" type="number" name="points_required" id="points_required" value="{{ old('points_required', $bid->points_required) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="featured" id="featured" value="1" {{ $bid->featured || old('featured', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('highlighted') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="highlighted" id="highlighted" value="1" {{ $bid->highlighted || old('highlighted', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('free') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="free" id="free" value="1" {{ $bid->free || old('free', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $bid->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('selected') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="selected" id="selected" value="1" {{ $bid->selected || old('selected', 0) === 1 ? 'checked' : '' }}>
