    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jobs.update", [$job->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('budget') ? 'is-invalid' : '' }}" name="budget_id" id="budget_id" required>
                    @foreach($budgets as $id => $entry)
                        <option value="{{ $id }}" {{ (old('budget_id') ? old('budget_id') : $job->budget->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id" required>
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $job->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('deadline') ? 'is-invalid' : '' }}" type="text" name="deadline" id="deadline" value="{{ old('deadline', $job->deadline) }}" required>
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('completed_files') ? 'is-invalid' : '' }}" id="completed_files-dropzone">
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('completed_link') ? 'is-invalid' : '' }}" type="text" name="completed_link" id="completed_link" value="{{ old('completed_link', $job->completed_link) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="published" id="published" value="1" {{ $job->published || old('published', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="published">{{ trans('cruds.job.fields.published') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job_code') ? 'is-invalid' : '' }}" type="text" name="job_code" id="job_code" value="{{ old('job_code', $job->job_code) }}" required>
